...
if ($user)
{
                ...
                <p class="form-up-img">
                    ...
                    <form action="' . $_DOMAIN . 'photos.php"method="POST" id="formUpImg" enctype="multipart/form-data" onsubmit="return false;">
                        ...
                        <div class="form-group box-pre-img hidden">
                            <p><strong>Xem ảnh trước</strong></p>
                        </div>
    ...
    else
    {
        ...
        if ($db->num_rows($sql_get_img))
        {
            echo '
                <div class="row list" id="list_img">
                    <div class="col-md-12">
                        <div class="checkbox"><label><input type="checkbox"> Chọn/Bỏ chọn hình ảnh</label></div>
                    </div>
            ';
            foreach($db->fetch_assoc($sql_get_img, 0) as $key => $data_img) 
            {
                ...
                if (file_exists('../'.$data_img['url'])) {
                    $status_img = '<label class="label label-success">True</label>';
                }
                else
                {
                    $status_img = '<label class="label label-danger">False</label>';
                }
        ...
        {
            echo '<br><br><div class="alert alert-info">Bạn chưa có hình ảnh nào.</div>';
        }   
    }
}
